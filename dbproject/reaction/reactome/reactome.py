...
if __name__ == 'reaction.reactome.reactome':
	ABSPATH = os.path.abspath(PATH('reaction','reactome'))
elif __name__ == 'reactome.reactome':
		...
		for i in xrange(0, len(reac_list)):
			...
			for m in xrange(0, len(sub_acc)):
				...
				if ID.objects.filter(external_id = sub_acc[m], source=Source("UniProt")):
					...
					print sub_acc[m]
				prot1 = UniprotEntry(accession=sub_acc[m])
				acc1 = prot1.accession()
				prot1.save(substrate=True, source=SOURCE)
				print 'Saved', acc1
				...
				for n in xrange(0, len(cata_acc)):
					...
					prot2 = UniprotEntry(accession=cata_acc[n])
					acc2 = prot2.accession()
					if p == 0:
						prot2.save(substrate=False, protein_type = 'K', source=SOURCE)
						Protein.objects.get(accession_number = str(prot2.accession())).protein_type = 'K'
					else:
						prot2.save(substrate=False, protein_type = 'P', source=SOURCE)
						Protein.objects.get(accession_number = str(prot2.accession())).protein_type = 'P'
					print 'Saved', prot2.accession()
